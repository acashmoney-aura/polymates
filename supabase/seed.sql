...
select l.id, em.id, null, 'manual', 'approved'
from leagues l
join external_markets em on em.external_id in ('bra-mor', 'usa-par', 'arg-advance') and em.source = 'polymarket'
where l.invite_code = 'START123'
on conflict do nothing;

insert into trade_intents (league_id, user_id, external_market_id, side, shares, estimated_price, estimated_cost, status)
select l.id, null, em.id, 'YES', 100, 66, 66, 'filled'
from leagues l
join external_markets em on em.external_id = 'bra-mor' and em.source = 'polymarket'
where l.invite_code = 'START123'
on conflict do nothing;

insert into settlement_actions (league_id, external_market_id, result, action_type, note, created_by)
select l.id, em.id, 'YES', 'resolve', 'Resolved YES and pushed league payout snapshot.', null
from leagues l
